...
fn dump_correction_arrays() {
    ...
    for &(iem, target_name) in CASES {
        ...
        println!(
            "Error (pre-equalize): {} points, db range [{err_min:.3}, {err_max:.3}]",
            error.len()
        );
        println!(
            "  First 3: {:?}",
            &error[..3]
                .iter()
                .map(|p| (p.freq.round(), (p.db * 1000.0).round() / 1000.0))
                .collect::<Vec<_>>()
        );
        println!(
            "  Last 3:  {:?}",
        ...
        println!(
            "Equalize output: {} points, db range [{eq_min:.3}, {eq_max:.3}]",
            eq.len()
        );
        println!(
            "  First 3: {:?}",
        ...
        let eq_102_max = eq_102
            .iter()
            .map(|p| p.db)
            .fold(f64::NEG_INFINITY, f64::max);
        println!(
            "Eq on 1.02 grid: {} points, db range [{eq_102_min:.3}, {eq_102_max:.3}]",
            eq_102.len()
        );

        // Write as JSON
        let json_points: Vec<String> = eq_102
            .iter()
            .map(|p| format!("  {{\"freq\": {:.6}, \"db\": {:.6}}}", p.freq, p.db))
            .collect();
        let json = format!("[\n{}\n]", json_points.join(",\n"));
