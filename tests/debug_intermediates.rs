...
fn band_stats(label: &str, fr: &[FreqPoint]) {
    let bands = [
        (20.0, 200.0, "sub-bass"),
        (200.0, 2000.0, "mid"),
        (2000.0, 8000.0, "upper-mid"),
        (8000.0, 20000.0, "treble"),
    ];
    let parts: Vec<String> = bands
        .iter()
        .filter_map(|&(lo, hi, name)| {
            let vals: Vec<f64> = fr
                .iter()
...
fn spacing_report(label: &str, filters: &[biquad_rust::types::Filter]) {
    let mut pk_fcs: Vec<f64> = filters
        .iter()
    ...
    println!(
        "  {label} PK fc (Hz): {:?}",
        pk_fcs.iter().map(|f| format!("{f:.0}")).collect::<Vec<_>>()
    );
    println!(
        "  {label} PK spacing (oct): {:?}  min={:.2}",
...
fn debug_pipeline_intermediates() {
    ...
    for &(iem, target_name, constraint_name) in FAILING {
        ...
        let opts102 = InterpolateOptions {
            step: Some(1.02),
            f_min: None,
            f_max: None,
        };
        ...
        let eq_on_grid: Vec<f64> = grid
            ...
            .collect();
        let our_mse: f64 = our_cascade
            .iter()
            .zip(&eq_on_grid)
            .map(|(c, e)| (c - e).powi(2))
            .sum::<f64>()
            / grid.len() as f64;
        let golden_mse: f64 = golden_cascade
            .iter()
            .zip(&eq_on_grid)
            .map(|(c, e)| (c - e).powi(2))
            .sum::<f64>()
