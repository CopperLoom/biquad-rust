...
use biquad_rust::optimize::{optimize_from_x0, compute_x0};
use biquad_rust::types::{Constraints, FilterSpec, FilterType, MinStd};
use helpers::{load_fr, load_target, optimizer_grid, rmse, load_golden};
use std::path::Path;

fn qudelix_10() -> Constraints {
    let mut specs = vec![FilterSpec {
        filter_type: Some(FilterType::LSQ), fc: None, q: None, gain: None,
        optimize_fc: None, optimize_q: None, optimize_gain: None,
        fc_range: None, q_range: Some((0.5, 10.0)), gain_range: (-12.0, 12.0),
    }];
    for _ in 0..8 { specs.push(FilterSpec {
        filter_type: Some(FilterType::PK), fc: None, q: None, gain: None,
        optimize_fc: None, optimize_q: None, optimize_gain: None,
        fc_range: None, q_range: Some((0.5, 10.0)), gain_range: (-12.0, 12.0),
    }); }
    specs.push(FilterSpec {
        filter_type: Some(FilterType::HSQ), fc: None, q: None, gain: None,
        optimize_fc: None, optimize_q: None, optimize_gain: None,
        fc_range: None, q_range: Some((0.5, 10.0)), gain_range: (-12.0, 12.0),
    });
...
fn standard() -> Constraints {
    Constraints {
        filter_specs: vec![
            FilterSpec { filter_type: Some(FilterType::LSQ), fc: None, q: None, gain: None,
                optimize_fc: None, optimize_q: None, optimize_gain: None,
                fc_range: None, q_range: Some((0.5, 10.0)), gain_range: (-12.0, 12.0) },
            ...
            FilterSpec { filter_type: Some(FilterType::PK), fc: None, q: None, gain: None,
                optimize_fc: None, optimize_q: None, optimize_gain: None,
                fc_range: None, q_range: Some((0.5, 10.0)), gain_range: (-12.0, 12.0) },
            FilterSpec { filter_type: Some(FilterType::HSQ), fc: None, q: None, gain: None,
                optimize_fc: None, optimize_q: None, optimize_gain: None,
                fc_range: None, q_range: Some((0.5, 10.0)), gain_range: (-12.0, 12.0) },
        ],
...
fn load_x0(name: &str) -> Vec<f64> {
    ...
    let j: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(&path)
        .unwrap_or_else(|_| panic!("Run same_x0_test.py first to generate {path:?}"))).unwrap();
    j["x0"].as_array().unwrap().iter().map(|v| v.as_f64().unwrap()).collect()
}
...
fn report(case: &str, constraints: &Constraints) {
    ...
    println!("\nOur x0:    {:?}", our_x0.iter().map(|v| format!("{v:.4}")).collect::<Vec<_>>());
    println!("AutoEQ x0: {:?}", load_x0(case).iter().map(|v| format!("{v:.4}")).collect::<Vec<_>>());

    println!("\nOur init filters:");
    for f in &our_init {
        println!("  {:3?}  fc={:8.1}  gain={:7.3}  q={:.3}", f.filter_type, f.fc, f.gain, f.q);
    }
    ...
    let rust_cascade: Vec<f64> = total_response(&result_from_autoeq_x0.filters, &freqs, 44100.0)
        .iter().map(|&v| v + result_from_autoeq_x0.pregain).collect();
    println!("\nRMSE (Rust-from-AutoEQ-x0 vs golden): {:.4} dB", rmse(&rust_cascade, &golden_cascade));

    // Verdict
    let our_x0_val = compute_x0(&measured, &target, constraints).unwrap().0;
    let autoeq_x0_val = load_x0(case);
    let x0_diff: f64 = our_x0_val.iter().zip(&autoeq_x0_val).map(|(a,b)| (a-b).abs()).fold(0.0_f64, f64::max);
